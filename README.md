...
Данный репозиторий содержит в себе мой диплом бакалавра, составленный
по шаблону [pavel-collab](https://github.com/pavel-collab/Bachelor-Thesis-Template)
согласно ГОСТам.
...
перед текстом, размер шрифта которого вы хотите изменить. В файле _main.tex_ уже стоит её применённая
версия. При необходимости изменить шрифт редактируйте её.

## Про размера отступа

Если хотите изменить расстояние между строчками --- используйте команду
```
\linespread{1.5}
```
В файле _preambule.tex_ уже стоит её применённая версия. При необходимости редактируйте её.
...
Далее описана структура из шаблона, структура моего диплома несколько отличается.
